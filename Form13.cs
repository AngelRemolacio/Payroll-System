    ...
    public partial class Registrarform : Form
    {
       ...
       (
        ...
        public void employee()
        {
            ...
            connection();
            string query = "SELECT ID_Number, CONCAT(FirstName, ' ', LastName) AS Name, Mobile, Address, Position, SSS_ID, PagIbig_ID, PhilHealth_ID, TIN_ID, Total_Work_Hours, Total_Over_Time, Night_Over_Time, Total_Contribution, Net_Pay, Gross_Pay FROM tblemployee WHERE Department = 'Registrar'";
            DataTable Table = new DataTable();
            MySqlDataAdapter mySqlAdapter = new MySqlDataAdapter(query, conn1);
            mySqlAdapter.Fill(Table);
            dgvReg.DataSource = Table;
